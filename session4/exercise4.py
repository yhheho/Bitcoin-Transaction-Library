from session4.ecc import G, N, S256Point, Signature, PrivateKey
from io import BytesIO
from session4.Tx import Tx, TxIn, TxOut
from session4.Script import Script
from session4.helper import double_sha256
from random import randint


class Exercise4:

    @staticmethod
    def print_divider():
        print("========================================")

    @staticmethod
    def exercise1_1():
        hex_scripts = '767695935687'
        s = Script.parse(bytes.fromhex(hex_scripts))
        print(s)

    @staticmethod
    def exercise2_1():
        Exercise4.print_divider()
        hex_scripts = '6e879169a77ca787'
        s = Script.parse(bytes.fromhex(hex_scripts))
        print(s)

    @staticmethod
    def exercise4_1():
        Exercise4.print_divider()
        prev_tx = bytes.fromhex('d1c789a9c60383bf715f3f6ad9d14b91fe55f3deb369fe5d9280cb1a01793f81')
        prev_index = 0

        tx_in = TxIn(prev_tx, 0, b'', 0xffffffff)
        t = tx_in.fetch_tx()
        prev_output = t.tx_outs[prev_index]
        print(prev_output.amount)
        print(prev_output.script_pubkey)

    @staticmethod
    def exercise5_1():
        Exercise4.print_divider()
        hex_tx = ''
        stream = BytesIO(bytes.fromhex(hex_tx))
        t = Tx.parse(stream)
        print(t.fee())

    @staticmethod
    def exercise5_2():
        Exercise4.print_divider()
        modified_tx = bytes.fromhex('')
        h = double_sha256(modified_tx)
        z = int.from_bytes(h, 'big')
        print(h.hex())

